...
def handle_callback(event: dict, origin: str) -> dict:
    ...
    # Get code, code_verifier, device_id and referral_code
        ...
        try:
            ...
            if row:
                ...
                if vk_email and row:
                    ...
                    # 2b. Check by phone from VK
                    vk_phone = user_info.get('phone', '')
                    row = None
                    if vk_phone:
                        cur.execute(
                            f"SELECT id, first_name, last_name, avatar_url FROM {S}users WHERE phone = %s AND phone != ''",
                            (vk_phone,)
                        )
                        row = cur.fetchone()
                    if vk_phone and row:
                        ...
                        email = vk_email or f"vk_{vk_user_id}@vk.local"
                        name = f"{db_first_name or ''} {db_last_name or ''}".strip() or full_name
                        photo_url = db_avatar or photo_url
                    else:
                        # 3. Create new user
