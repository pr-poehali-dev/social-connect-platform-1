                ...
                if email and row:
                    ...
                    cur.execute(
                        f"""UPDATE {S}users
                            ...
                            WHERE id = %s""",
                        (yandex_id, picture, name, now, now, user_id)
                    )
                    name = db_name or name
                    picture = db_avatar or picture
                else:
                    # 2b. Check by phone from Yandex
                    yandex_phone = user_info.get('default_phone', {})
                    yandex_phone_number = yandex_phone.get('number', '') if isinstance(yandex_phone, dict) else ''
                    row = None
                    if yandex_phone_number:
                        cur.execute(
                            f"SELECT id, name, avatar_url FROM {S}users WHERE phone = %s AND phone != ''",
                            (yandex_phone_number,)
                        )
                        row = cur.fetchone()
                    if yandex_phone_number and row:
                        user_id, db_name, db_avatar = row
                        cur.execute(
                            f"""UPDATE {S}users
                                SET yandex_id = %s,
                        ...
                        cur.execute(
                            f"""INSERT INTO {S}users
                                (yandex_id, email, password_hash, name, nickname, avatar_url, email_verified, created_at, updated_at, last_login_at, referred_by)
                                VALUES (%s, %s, %s, %s, %s, %s, TRUE, %s, %s, %s, %s)
                                RETURNING id""",
                            (yandex_id, email, '', name, nickname, picture, now, now, now, referrer_id)
                        )
