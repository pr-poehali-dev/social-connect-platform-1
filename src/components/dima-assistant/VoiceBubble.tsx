...
import { DIMA_AVATAR, TTS_URL, MOOD_LABELS, MOOD_COLORS, type VoiceMood } from './constants';
...
const VoiceBubble = ({ text, mood = 'default' }: { text: string; mood?: VoiceMood }) => {
  ...
  const loadAudio = async (): Promise<HTMLAudioElement> => {
    ...
    const res = await fetch(TTS_URL, {
      ...
      body: JSON.stringify({ text, mood, voice: 'ru-RU-DmitryNeural' }),
    });
    ...
    <button onClick={play} className="flex items-center gap-2 group min-w-[160px]">
      <div className="relative w-10 h-10 shrink-0">
        <img
          src={DIMA_AVATAR}
          alt="voice"
          className={`w-10 h-10 rounded-full object-cover border-2 ${isPlaying ? colors.border : 'border-blue-300'} transition-colors`}
        />
        {isPlaying && (
