...
const HoroscopeWidget = () => {
  ...
  if (!sign) {
    return (
      <Card className="rounded-3xl border-2 overflow-hidden cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate('/horoscope')}>
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Укажите знак зодиака в профиле, чтобы видеть ежедневный гороскоп</p>
            </div>
      ...
      <CardContent className="p-4">
        {horoscope ? (
          <div className="space-y-3">
            <p className="text-sm leading-relaxed line-clamp-3">{horoscope.content}</p>
            ...
            >
              ...
              Натальная карта, совместимость и ещё
            </Button>
          </div>
        ) : (
          <div className="text-center py-2">
            <p className="text-sm text-muted-foreground">Не удалось загрузить гороскоп</p>
            <Button variant="link" size="sm" onClick={() => navigate('/horoscope')}>Открыть астрологию</Button>
          </div>
        )}
      </CardContent>
    </Card>
